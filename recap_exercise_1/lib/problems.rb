...
def all_vowel_pairs(words)
    ...
    (0...words.length).each do |idx_1|
        ((idx_1 + 1)...words.length).each do |idx_2|
            pair = words[idx_1] + " " + words[idx_2]
            word_pairs << pair if vowels.all? {|char| pair.include?(char)}
        end
    end
...
def find_bigrams(str, bigrams)
    ...
    bigrams.select { |ele| all_bigrams.include?(ele)}
end
...
class String
    ...
    def substrings(length = nil)
        ...
        if length == nil
            ...
            my_arr.select {|sub| sub.length == length}
        end
    end
